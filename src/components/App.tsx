...
export function App() {
  ...
  return (
    <div class="max-md:relative md:grid md:grid-cols-2">
      {/* Palette */}
      <div class="mt-52 flex flex-col gap-2 py-4 md:col-start-1 md:mt-0 md:h-screen md:overflow-auto">
        {/* Scheme Bar */}
        {colors.map(({ scheme, cells }) => (
          <div>
            ...
            <h2
              id={scheme}
              class="mb-2 font-semibold tracking-wide max-md:scroll-mt-56"
            >
      ...
      <div class="flex flex-col items-center justify-center py-4 max-md:fixed max-md:top-0 max-md:left-0 max-md:right-0 max-md:border-b max-md:bg-white md:col-start-2 md:h-screen">
        {/* Card */}
        <div class="flex flex-col items-center justify-between gap-2 bg-white md:min-w-max md:gap-4 md:rounded-lg md:border md:py-6 md:px-10 md:shadow-xl">
          {/* Theme Title */}
          <h2
            id="currThemeTitle"
          ...
          {/* External Links */}
          <a
            class="aspect-square w-6 text-slate-600 transition-colors hover:text-slate-500"
            href="https://github.com/3fuyang/tailwind-default-palette"
            target="_blank"
            rel="noreferrer"
            title="GitHub"
          >
